...
use crate::sumcheck::zk::{ZkSumcheckData, ZkSumcheckHandoff};
...
impl<F: Field, EF: ExtensionField<F>> SumcheckProver<F, EF> {
        ...
        Ch: FieldChallenger<F> + GrindingChallenger<Witness = F> + CanObserve<M::Commitment>,
    {
        ...
        let masks: Vec<Vec<F>> = (0..folding_factor)
        ...
        let sum_endpoints_init: F = masks
            ...
            .sum();
        let two_to_k_minus_1 = F::TWO.exp_u64((folding_factor - 1) as u64);
        ...
        let pow2: Vec<F> = F::TWO.powers().collect_n(folding_factor + 1);

        for (round_idx, mask) in masks.iter().enumerate().take(folding_factor) {
            let j = round_idx + 1;
            let mask_endpoints = mask[0].double() + mask[1..].iter().copied().sum::<F>();
